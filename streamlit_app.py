...
for route_id in route_ids:
    ...
    legend_html += (
        f"<div title='{route_name}' style='display: flex; align-items: center; gap: 0.5rem;'>"
...
# ----------------------------
# Centra la mappa su linee attive
# ----------------------------
selected_shapes = trips[trips["route_id"].isin(selected_routes)]["shape_id"].unique()
selected_points = shapes[shapes["shape_id"].isin(selected_shapes)]
if not selected_points.empty:
    map_center = [selected_points["lat"].mean(), selected_points["lon"].mean()]
else:
